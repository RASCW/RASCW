...
Begin VB.Form frmwellPlot2 
   Caption         =   " CASC Correlation - Scaling"
   ClientHeight    =   8835
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   12360
   Icon            =   "frmwellPlot2.frx":0000
   LinkTopic       =   "Form1"
   MDIChild        =   -1  'True
   ScaleHeight     =   8835
   ScaleWidth      =   12360
   ...
   Begin OlectraChart2D.Chart2D ChartWell 
      ...
      _ExtentY        =   14203
      _StockProps     =   0
      ControlProperties=   "frmwellPlot2.frx":0442
   End
   ...
   End
End
Attribute VB_Name = "frmwellPlot2"
...
Dim I, J As Integer
    ...
    If Not (frmDataTable2 Is Nothing) Then
       Unload frmDataTable2
    End If

    If chkTable.Value = 1 Then
     'get values for data table()
     For J = 1 To max_event_num2 'ratio values for data table
       TableRatio2(J) = ratio(J)
     Next J
     ' data table values
     For I = 1 To max_well_num2
       For J = 1 To max_event_num2
        TableObs2(I, J) = ObsDepth(I, J)
        TableMinObs2(I, J) = MinObs(I, J)
        TableMaxObs2(I, J) = MaxObs(I, J)
        TableProb2(I, J) = probDepth(I, J)
        TableMinProb2(I, J) = MinDepth(I, J)
        TableMaxProb2(I, J) = maxDepth(I, J)
       Next J
       Next I
...
Private Sub cmdModify_Click()
    ...
    For J = 1 To max_event_num2
      list_flattening.AddItem Space(2 * (4 - Len(Str(event_numbers2(J))))) + Str(event_numbers2(J)) + Space(5) + event_names2(J)
    Next J
...
Private Sub cmdOpen_Click()
    ...
    modifyflag2 = 0 ' for modifying chart
    ...
    If txt_file_ran1 = "" Or testing_flag2 = 0 Then
      Beep
      Exit Sub
    End If
    ' set initial for flattening number = 0
...
Dim new_I As Integer
 ...
 '   For I = 1 To max_well_num2
 '       MsgBox Str(I) + " original order" + Str(orders2(I)) + " well " + Str(well_numbers2(I))
 '  Next I
...
ReDim orders2(1 To max_well_num2)
'sort well_numbers2(I)  max_well_num2 is from use selected number in "frmWells2.frm"
For I = 1 To max_well_num2    ' initialize the orders2 array
     orders2(I) = I
Next

' sort well_numbers2 by linear sorting
   If max_well_num2 > 1 Then
     For I = 1 To max_well_num2
         For J = 1 To max_well_num2 - 1
           If well_numbers2(J) > well_numbers2(J + 1) Then
                test_num = well_numbers2(J)
                well_numbers2(J) = well_numbers2(J + 1)
                well_numbers2(J + 1) = test_num
                test_num = orders2(J)
                orders2(J) = orders2(J + 1)
                orders2(J + 1) = test_num
            End If
         Next J
    Next I
   End If
...
ReDim ObsDepth(1 To max_well_num2, 1 To max_event_num2) As Integer
ReDim MinObs(1 To max_well_num2, 1 To max_event_num2) As Integer
ReDim MaxObs(1 To max_well_num2, 1 To max_event_num2) As Integer
ReDim probDepth(1 To max_well_num2, 1 To max_event_num2) As Integer
ReDim MinDepth(1 To max_well_num2, 1 To max_event_num2) As Integer
ReDim maxDepth(1 To max_well_num2, 1 To max_event_num2) As Integer
ReDim ratio(1 To max_event_num2) As Double
...
III = 0
For I = 1 To max_well_num2       'every wells
     If (well_numbers2(I) - III) > 1 Then
        ' (act_Num_event + 8)  ---->  data lines of each well in datafile
        For J = 1 To (well_numbers2(I) - III - 1) * (act_Num_event + 8)
           Input #FileNum, Temp
        Next J
      End If
      III = well_numbers2(I)
      ...
      Line Input #FileNum, Temp
        ...
        If test_num <> well_numbers2(I) Then
...
' back to original orders2 of well numbers
...
If flattening_num <> 0 Then
  ...
  For J = 1 To max_event_num2
     If event_numbers2(J) = flattening_num Then
...
If numOfflattening <> 0 And flattening_num <> 0 Then
 ...
 For I = 1 To max_well_num2
    If prob_opt2 = True Then
        If probDepth(I, numOfflattening) = -1 Then
            If ObsDepth(I, numOfflattening) = -1 Then
               TempProbDepth = InputBox("Probable and observed depths do not exist for " + well_label_used2(I), _
                ...
                Wend
            Else
              TempProbDepth = InputBox("Probable depth does not exist for " + well_label_used2(I) + Chr(13) + _
    ...
    ElseIf norm_opt2 = True Or obs_opt2 = True Then
      
        If ObsDepth(I, numOfflattening) = -1 Then
            If probDepth(I, numOfflattening) = -1 Then
              TempObsDepth = InputBox("Probable and observed depths do not exist for " + well_label_used2(I), _
            ...
            Else
              TempObsDepth = InputBox("Observed depth does not exist for " + well_label_used2(I) + Chr(13) + _
...
'ReDim newwell(1 To max_well_num2) ' back to the original order for labelling
'For I = 1 To max_well_num2
'new_I = orders2(I)
'    newwell(I) = well_numbers2(new_I)
'Next I
...
ChartWell.ChartArea.Axes("x").Max.Value = max_well_num2 - 1 + well_width
...
' design plot area
  If max_well_num2 < 6 Then
      ChartWell.ChartArea.PlotArea.left.Value = 90 * (6 - max_well_num2)
      ChartWell.ChartArea.PlotArea.right.Value = 90 * (6 - max_well_num2)
  End If

'desgin line types


    For I = 1 To 2 * max_event_num2 * max_well_num2 + 2 * max_event_num2
...
' add event numbers to the the well at the middle
 'middle = Int(max_well_num2 / 2) ' get the middle well
 
num_well_event = max_well_num2 + max_event_num2 * max_well_num2

For I = 1 To num_well_event  'max_well_num2 + max_event_num2
 ...
 If I <= max_well_num2 Then
    ChartWell.ChartLabels(I).Anchor = oc2dAnchorNorth
    If I <> max_well_num2 Then
     ChartWell.ChartLabels(I).AttachDataCoord.X = orders2(I) - 1
    Else
    ChartWell.ChartLabels(I).AttachDataCoord.X = orders2(I) - 1
    End If
     ' Label well number or name
     ChartWell.ChartLabels(I).AttachDataCoord.Y = allmindepth - YExtendLength
     ChartWell.ChartLabels(I).Text.Text = well_label_used2(I)
 ...
 Else
        ...
        'middle > 0 Then
         If max_well_num2 = 1 Then
           column = 1
         Else
            column = I Mod max_well_num2 'reminder of number of well
         End If
        ...
        ' add labels for the first well
         ...
         If column = 1 Then
            If prob_opt2 = True And probDepth(column, row) > -1 Then
            ...
            '    ChartWell.ChartLabels(i).AttachDataCoord.y = probDepth(column, row)
            '    End If
                 ChartWell.ChartLabels(I).Text.Text = Str(event_numbers2(row))
             End If
             
             If norm_opt2 = True And ObsDepth(column, row) > -1 Then
            ...
            '    If ObsDepth(column, row) < ObsDepth(column + 1, row) Then
             ...
             If obs_opt2 = True And ObsDepth(column, row) > -1 Then
                ChartWell.ChartLabels(I).Anchor = oc2dAnchorEast
                ChartWell.ChartLabels(I).AttachDataCoord.X = column - 1 + well_width * width_ceo
            '    If ObsDepth(column, row) < ObsDepth(column + 1, row) Then
        ...
        If column = 0 Then
         ...
         'add labels to the wells in middle
          ...
          If column > 1 And column < max_well_num2 Then
            If prob_opt2 = True And probDepth(column, row) > -1 Then
...
tempTitle = Mid(txt_file_ran1, 1, tempPost - 1)
   ...
   If prob_opt2 = True Then
        ...
        Table_Title2 = title_str + " - File: " + tempTitle
   ElseIf norm_opt2 = True Then
        ChartWell.Header.Text.Text = "Observed Depths with Error Bars (" + title_str + "; " + tempTitle + ")"
        Table_Title2 = title_str + " - File: " + tempTitle
   ElseIf obs_opt2 = True Then
...
ChartWell.ChartArea.Axes("x").Max.Value = max_well_num2 - 1 + well_width
ChartWell.ChartArea.Axes("x").Min.Value = -well_width
ChartWell.ChartArea.Axes("x").DataMax.Value = max_well_num2 - 1
...
With ChartWell.ChartGroups(2).Data
    ...
    .IsBatched = True
    .NumSeries = 2 * max_event_num2 * max_well_num2 + 2 * max_event_num2
   
   ' inilitialize the points per series
   
      For I = 1 To 3 * max_event_num2
       .NumPoints(I) = max_well_num2
      Next I
      
      For I = 1 To 2 * max_event_num2 * max_well_num2 - max_event_num2
       .NumPoints(I + 3 * max_event_num2) = 2
      Next I
   ...
   'plot min depth as bars
           ...
           If prob_opt2 = True Then
                    For J = 1 To max_well_num2
                        If MinDepth(J, I) > -1 Then
                            .X(I + max_event_num2, J) = J - 1
                                If numOfflattening <> 0 Then
                                 .Y(I + max_event_num2, J) = PlotMindepth(J, I)
                                 Else
                                .Y(I + max_event_num2, J) = MinDepth(J, I)
                                End If
                         End If
                     Next J
             ...
             ElseIf norm_opt2 = True Then
                     For J = 1 To max_well_num2
                         If MinObs(J, I) > -1 Then
                            .X(I + max_event_num2, J) = J - 1
                               If numOfflattening <> 0 Then
                                .Y(I + max_event_num2, J) = PlotMinObs(J, I)
                                Else
                                .Y(I + max_event_num2, J) = MinObs(J, I)
                                End If
                           End If
                      Next J
              End If
              ...
              If prob_opt2 = True Then
                    For J = 1 To max_well_num2
                        If maxDepth(J, I) > -1 Then
                           .X(I + 2 * max_event_num2, J) = J - 1
                           If numOfflattening <> 0 Then
                           .Y(I + 2 * max_event_num2, J) = PlotMaxdepth(J, I)
                            Else
                            .Y(I + 2 * max_event_num2, J) = maxDepth(J, I)
                           End If
                         End If
                     Next J
               ...
               ElseIf norm_opt2 = True Then
                    For J = 1 To max_well_num2
                         If MaxObs(J, I) > -1 Then
                            .X(I + 2 * max_event_num2, J) = J - 1
                            If numOfflattening <> 0 Then
                            .Y(I + 2 * max_event_num2, J) = PlotMaxObs(J, I)
                            Else
                            .Y(I + 2 * max_event_num2, J) = MaxObs(J, I)
                            End If
                         End If
                    Next J
                End If
    ...
    acumIndex = 0
     For I = 1 To max_event_num2
         For J = 1 To max_well_num2
            'skip no data points
              If prob_opt2 = True And MinDepth(J, I) > -1 And maxDepth(J, I) > -1 Then
                 acumIndex = acumIndex + 1
                .X(acumIndex + 3 * max_event_num2, 1) = J - 1
                .X(acumIndex + 3 * max_event_num2, 2) = J - 1
                    If numOfflattening <> 0 Then
                    .Y(acumIndex + 3 * max_event_num2, 1) = PlotMindepth(J, I)
                    .Y(acumIndex + 3 * max_event_num2, 2) = PlotMaxdepth(J, I)
                    Else
                    .Y(acumIndex + 3 * max_event_num2, 1) = MinDepth(J, I)
                    .Y(acumIndex + 3 * max_event_num2, 2) = maxDepth(J, I)
                    End If
               End If
              If norm_opt2 = True And ObsDepth(J, I) > -1 And MinObs(J, I) > -1 And MaxObs(J, I) > -1 Then
                acumIndex = acumIndex + 1
                .X(acumIndex + 3 * max_event_num2, 1) = J - 1
                .X(acumIndex + 3 * max_event_num2, 2) = J - 1
                    If numOfflattening <> 0 Then
                     .Y(acumIndex + 3 * max_event_num2, 1) = PlotMinObs(J, I)
                     .Y(acumIndex + 3 * max_event_num2, 2) = PlotMaxObs(J, I)
                    Else
                    .Y(acumIndex + 3 * max_event_num2, 1) = MinObs(J, I)
                    .Y(acumIndex + 3 * max_event_num2, 2) = MaxObs(J, I)
                    End If
             End If
       Next J
     Next I
    ...
    For I = 1 To max_event_num2
         ...
         'skip no data points
         If prob_opt2 = True And probDepth(J, I) > -1 And probDepth(J + 1, I) > -1 Then
         acumIndex = acumIndex + 1
        .X(acumIndex + 3 * max_event_num2 + max_event_num2 * max_well_num2, 1) = J - 1 + well_width * 0.5
        .X(acumIndex + 3 * max_event_num2 + max_event_num2 * max_well_num2, 2) = J - well_width * 0.5
           If numOfflattening <> 0 Then
            .Y(acumIndex + 3 * max_event_num2 + max_event_num2 * max_well_num2, 1) = Plotprobdepth(J, I)
            .Y(acumIndex + 3 * max_event_num2 + max_event_num2 * max_well_num2, 2) = Plotprobdepth(J + 1, I)
           Else
            .Y(acumIndex + 3 * max_event_num2 + max_event_num2 * max_well_num2, 1) = probDepth(J, I)
            .Y(acumIndex + 3 * max_event_num2 + max_event_num2 * max_well_num2, 2) = probDepth(J + 1, I)
           End If
         End If
         If prob_opt2 = False And ObsDepth(J, I) > -1 And ObsDepth(J + 1, I) > -1 Then
              acumIndex = acumIndex + 1
        .X(acumIndex + 3 * max_event_num2 + max_event_num2 * max_well_num2, 1) = J - 1 + well_width * 0.5
        .X(acumIndex + 3 * max_event_num2 + max_event_num2 * max_well_num2, 2) = J - well_width * 0.5
           If numOfflattening <> 0 Then
             .Y(acumIndex + 3 * max_event_num2 + max_event_num2 * max_well_num2, 1) = PlotObsdepth(J, I)
             .Y(acumIndex + 3 * max_event_num2 + max_event_num2 * max_well_num2, 2) = PlotObsdepth(J + 1, I)
           Else
             .Y(acumIndex + 3 * max_event_num2 + max_event_num2 * max_well_num2, 1) = ObsDepth(J, I)
             .Y(acumIndex + 3 * max_event_num2 + max_event_num2 * max_well_num2, 2) = ObsDepth(J + 1, I)
            End If
         End If
       Next J
     Next I
...
'????????????????????????
'For I = 1 To 3 * max_event_num2 + max_event_num2 * max_well_num2
 ' ChartWell.ChartGroups(2).Styles(1).Line.Width = 2
  '    ChartWell.ChartGroups(2).Styles(1).Line.Color = RGB(255, 0, 0)
'Next I
...
Dim size As Integer
If (frmwellPlot2.Height > 360) And (frmwellPlot2.Height - 360 < frmwellPlot2.Width / 1.5) Then
   size = frmwellPlot2.Height - 360
ElseIf (frmwellPlot2.Height > 360) And (frmwellPlot2.Height - 360 > frmwellPlot2.Width / 1.5) Then
    size = frmwellPlot2.Width / 1.5
Else
    size = 0
End If
...
ChartWell.Visible = False
flattening_num = event_numbers2(list_flattening.ListIndex + 1)
startup
List_Well
End Sub
